# 🏥 Healthcare Analytics Dashboard — Power BI

A multi-page Power BI dashboard analyzing 55K+ hospital records across 
patient demographics, operations, finance, and medical insights.

---

## 🛠️ Tools Used
- Power BI Desktop

## 📂 Data Source
Dataset obtained from [Kaggle](https://www.kaggle.com)

## 💡 Key Insights
- Diabetes is the biggest cost driver at $236.49M in billing
- Billing peaked in 2021–2022 and dropped sharply in 2024
- Admissions are evenly split across elective, emergency, and urgent types
- Older patients (70–80) make up the largest share of admissions

---

## 📸 Screenshots

### Main Dashboard
![Dashboard](Dashboard.png)
*Total patients, admissions, billing overview, age and gender distribution.*

### Patients Demographics
![Patients](Patients.png)
*Patient breakdown by age, gender, blood type and medical condition.*

### Hospital Operations
![Hospital Operations](Hospital%20Operations.png)
*Length of stay, top doctors, admission types and hospital turnover.*

### Finance & Insurance
![Finance & Insurance](Finance%20&%20Insurance.png)
*Billing per patient, insurance provider comparison and top cost-driving conditions.*

### Medical Insight
![Medical Insight](Medical%20Insight.png)
*Test results, common conditions, top medications and billing impact by age.*

---

**Daniel Agbaso** — Data Analyst | Power BI

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/danielchigozirim) [![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/agbaso-BI)
